// 예: SVG 렌더링용 WASM (resvg 기반)
import initResvg, { Resvg } from "@resvg/resvg-wasm";
// 예: WebP 인코더용 WASM
import initWebp, { encode } from "@jsquash/webp";
...
// SVG → WebP (WASM 기반)
async function svgToWebpWasm(svgString, quality = 80) {
  await ensureWasmReady();

  // 1) SVG → RGBA 버퍼 렌더링
  const resvg = new Resvg(svgString, {
    fitTo: {
      mode: "original", // 또는 width/height 등 옵션
    },
  });

  const rendered = resvg.render();
  const { width, height, data } = rendered; // data: Uint8Array (RGBA)

  // 2) RGBA → WebP 인코딩
  // encode() API는 사용하는 라이브러리에 따라 다를 수 있음
  const webpBytes = await encode(
    { data, width, height },  // 입력 이미지
    { quality }               // WebP 옵션
  );

  return webpBytes; // Uint8Array
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    // SVG → WebP (WASM 기반 변환)
    const webpBytes = await svgToWebpWasm(svg, 80);
